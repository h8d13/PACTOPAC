...
def check_audio_utils(auto_install=False):
    ...
    sof_status = check_installed("sof-firmware", auto_install)
    ...
    return {"alsa-utils": alsa_status, "sof-firmware": sof_status}

def check_kvm_support():
    print("\n--- Checking KVM Support ---")
    try:
        result = subprocess.run(["lsmod | grep kvm"], capture_output=True, text=True, shell=True)
        if result.returncode == 0 and result.stdout.strip():
            print("✓ KVM modules are loaded:")
            print(result.stdout.strip())
            return True
        else:
            print("✗ KVM modules not found")
            return False
    except:
        print("✗ Failed to check KVM modules")
        return False

def run_post_install_commands(category):
    if not DRIVERS_CONFIG:
        return False
    
    category_config = DRIVERS_CONFIG.get(category, {})
    if not isinstance(category_config, dict):
        return False
    
    commands = category_config.get("post_install_commands", [])
    if not commands:
        return True
    
    print(f"\n--- Running {category} post-install commands ---")
    
    for cmd in commands:
        print(f"Running: {cmd}")
        try:
            # Replace $USER with actual username
            expanded_cmd = cmd.replace("$USER", os.getenv("USER", ""))
            result = subprocess.run(expanded_cmd, shell=True, capture_output=True, text=True)
...
def check_virtualization_drivers(auto_install=False):
    ...
    kvm_supported = check_kvm_support()
    
    virt_config = DRIVERS_CONFIG.get("virtualization", {})
    if isinstance(virt_config, dict):
        drivers = virt_config.get("packages", [])
    else:
        # Fallback for old format
        drivers = virt_config if isinstance(virt_config, list) else []
    
    results = {}
    any_installed = False
    
    for driver in drivers:
        status = check_installed(driver, auto_install)
        results[driver] = status
        if status in ['installed', 'installed_now']:
            any_installed = True
    
    # Run post-install commands only if auto_install=True and packages are installed and KVM supported
    if auto_install and any_installed and kvm_supported:
        run_post_install_commands("virtualization")
    
    results["kvm_support"] = kvm_supported
    return results
