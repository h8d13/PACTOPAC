...
class PkgMan(Adw.ApplicationWindow):
    ...
    def cleanup_processes(self):
        ...
        # Clean up pacman lock file if it exists and we have permission
        try:
            lock_file = "/var/lib/pacman/db.lck"
            if os.path.exists(lock_file):
                os.remove(lock_file)
                print("Removed pacman lock file")
        except PermissionError:
            print("No permission to remove pacman lock file")
        except Exception as e:
    ...
    def show_settings(self, button):
        ...
        refresh_menu_row = Adw.ActionRow(
            title="Refresh KDE Menu",
            subtitle="Refresh the application menu to show newly installed Flatpak apps"
        )

        refresh_menu_btn = Gtk.Button(label="Refresh")
        refresh_menu_btn.add_css_class("suggested-action")
        refresh_menu_btn.set_valign(Gtk.Align.CENTER)
        refresh_menu_btn.connect("clicked", self.handle_menu_refresh)
        refresh_menu_row.add_suffix(refresh_menu_btn)

        flatpak_group.add(refresh_menu_row)
    ...
    def handle_menu_refresh(self, button):
        """Handle manual KDE menu refresh"""
    ...
    def install_fp_and_refresh(self, dialog):
        ...
        def check_and_reopen():
            ...
            def check_installation_complete():
                if self.check_fp():
                    # Flatpak is now installed, reopen settings dialog
    ...
    def refresh_kde_menu(self):
        ...
        try:
            ...
            self.show_info("KDE menu refresh initiated.\nNew Flatpak apps should appear in the application menu shortly.")
        except Exception as e:
            self.show_error(f"KDE menu refresh failed: {e}")
